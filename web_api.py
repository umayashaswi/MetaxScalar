...
@app.post("/v1/reset")
def reset_v1(req: ResetRequest):
    """OpenEnv compatible reset endpoint"""
    return reset(req)

@app.post("/v1/step")
def step_v1(req: StepRequest):
...
@app.post("/v1/reset")
def reset_v1(req: ResetRequest):
    """OpenEnv compatible reset endpoint - matches expected format"""
    task_id = req.task_id
    env = CustomerSupportEnv(task_id)
    env.reset()
    session_id = str(uuid.uuid4())
    ...
    # Return format that OpenEnv checker expects
    return {
        "session_id": session_id,
        "task_id": task_id,
        "max_score": max_score,
